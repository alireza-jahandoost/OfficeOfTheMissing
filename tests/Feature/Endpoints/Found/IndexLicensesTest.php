...
class IndexLicensesTest extends TestCase
{
    ...
    protected const FOUND_LICENSES_INDEX = 'licenses.founds.index_licenses';
    ...
    public function test_user_can_see_index_licenses_page()
    {
        ...
        $response = $this->actingAs($user)->get(route(self::FOUND_LICENSES_INDEX));

        $response->assertOk();

        $response->assertInertia(fn(AssertableInertia $page) => $page
            ->component('Founds/IndexLicenses')
            ->has('licenses', 5)
    ...
    public function test_guest_user_can_not_list_licenses()
    {
        ...
        $response = $this->get(route(self::FOUND_LICENSES_INDEX));

        $response->assertRedirect(route('login'));
    }

    public function test_admin_user_can_not_access_founds_index_licenses()
    {
        $licenses = License::factory()->count(5)->create();
        ...
        $response = $this->actingAs($admin)->get(route(self::FOUND_LICENSES_INDEX));
